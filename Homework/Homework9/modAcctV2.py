...
.format(self._tmpAcctNum,self._tmpBankName,self._tmpAcctType,self._tmpBalance)
	...
	def setBalance(self,newBalance):
		if self._tmpBalance!=0:
			intrestRate = (newBalance-self._tmpBalance)/self._tmpBalance
		else:
			intrestRate = 1
		self._tmpIntrest = intrestRate*self._tmpBalance
		self._tmpBalance = newBalance
		
	def computeIntrest(self,intrestRate):
		self._tmpIntrest = intrestRate*self._tmpBalance
		newBalance = self._tmpBalance*(1+intrestRate)
		self.setBalance(newBalance)
		return "${:<.2f}".format(self._tmpIntrest)
